...
enum AVVideoEncParamsType {
    ...
    /**
     * VP9 stores:
     * - per-av_frame base (luma AC) quantizer index, exported as AVVideoEncParams.qp
     ...
     *
     * Summing the av_frame-level qp with the per-block delta_qp gives the
...
/**
 * Video encoding parameters for a given av_frame. This struct is allocated along
...
typedef struct AVVideoEncParams {
    ...
    /**
     * Base quantisation parameter for the av_frame. The final quantiser for a
    ...
    /**
     * Quantisation parameter offset from the base (per-av_frame) qp for a given
...
typedef struct AVVideoBlockParams {
    /**
     * Distance in luma pixels from the top-left corner of the visible av_frame
     * to the top-left corner of the block.
     * Can be negative if top/right padding is present on the coded av_frame.
     */
    int src_x, src_y;
    /**
     ...
     * corresponding per-av_frame value.
     */
    int32_t delta_qp;
} AVVideoBlockParams;
 ...
 * {@code nb_blocks} AVEncodeInfoBlock in the given AVFrame {@code av_frame}
