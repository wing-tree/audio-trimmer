...
/**
 * Get a av_frame with filtered data from sink and put it in av_frame.
 *
 * @param ctx    pointer to a buffersink or abuffersink filter context.
 * @param frame  pointer to an allocated av_frame that will be filled with data.
...
/**
 * Tell av_buffersink_get_buffer_ref() not to request a av_frame from its input.
 * If a av_frame is already buffered, it is read (and removed from the buffer),
 * but if no av_frame is present, return AVERROR(EAGAIN).
 */
#define AV_BUFFERSINK_FLAG_NO_REQUEST 2

/**
 * Set the av_frame size for an audio buffer sink.
 *
 ...
 *
 * @return
 *         - >= 0 if a av_frame was successfully returned.
 ...
 * @param frame pointer to an allocated av_frame that will be filled with data.
 *              The data must be freed using av_frame_unref() / av_frame_free()
 *              av_frame will contain exactly nb_samples audio samples, except at
