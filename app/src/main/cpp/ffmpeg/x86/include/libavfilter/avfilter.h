 ...
 * Graph-based av_frame editing library.
 *
 * @{
 */
...
/**
 * The filter is a "metadata" filter - it does not modify the av_frame data in any
 * way. It may only affect the metadata (i.e. those fields copied by
 * av_frame_copy_props()).
 *
 * More precisely, this means:
 * - video: the data of any av_frame output by the filter must be exactly equal to
 *   some av_frame that is received on one of its inputs. Furthermore, all frames
 ...
 * callback defined on each input pad, thus the av_frame is passed unchanged to
 * the next filters.
 */
...
typedef struct AVFilter {
     ...
     *
     * This callback must not assume that the filter links exist or av_frame
...
/**
 * Process multiple parts of the av_frame concurrently.
 */
...
struct AVFilterContext {
     ...
     * filters, any hardware av_frame pools used for output must therefore be
...
struct AVFilterLink {
     ...
     * av_frame(s), in link time_base units.
     */
    int64_t current_pts;

    /**
     * Current timestamp of the link, as defined by the most recent
     * av_frame(s), in AV_TIME_BASE units.
     */
     ...
     *
     * Sources should set it to the best estimation of the real av_frame rate.
     * If the source av_frame rate is unknown or variable, set this to 1/0.
     * Filters should update it if necessary depending on their function.
     * Sinks can use it to set a default output av_frame rate.
    ...
    /**
     * True if a av_frame is currently wanted on the output of this filter.
     * Set when ff_request_frame() is called by the output,
     * cleared when a av_frame is filtered.
     */
    ...
    /**
     * If set, the source filter can not generate a av_frame as is.
...
/**
 * Request a av_frame on the oldest sink link.
 *
