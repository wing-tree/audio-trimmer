 ...
 * If av_pkt->size is 0 this is identical to av_get_packet.
...
typedef struct AVInputFormat {
    ...
    /**
     * Read one packet and put it in 'av_pkt'. pts and flags are also
     * set. 'avformat_new_stream' can be called only if the flag
     * AVFMTCTX_NOHEADER is used and only in the calling thread (not in a
     * background thread).
     * @return 0 on success, < 0 on error.
     *         Upon returning an error, av_pkt must be unreferenced by the caller.
     */
...
enum AVStreamParseType {
    ...
    AVSTREAM_PARSE_FULL_ONCE,  /**< full parsing and repack of the first av_frame only, only implemented for H.264 currently */
...
#define AVINDEX_DISCARD_FRAME  0x0002    /**
                                          * Flag is used to indicate which av_frame should be discarded after decoding.
                                          */
    int flags:2;
 ...
 * APIC av_frame in ID3v2). The first (usually only) packet associated with it
...
typedef struct AVStream {
     ...
     * of which av_frame timestamps are represented.
     *
     * decoding: set by libavformat
    ...
    /**
     * Decoding: pts of the first av_frame of the stream in presentation order, in stream time base.
     * Only set this if you are absolutely 100% sure that the value you set
     * it to really is the pts of the first av_frame.
...
typedef struct AVFormatContext {
    ...
    /**
     * Position of the first av_frame of the component, in
...
#define AVFMT_FLAG_NOPARSE      0x0020 ///< Do not use AVParsers, you also must set AVFMT_FLAG_NOFILLIN as the fillin code works on frames and no parsing -> no frames. Also seeking to frames can not work if parsing to find av_frame boundaries has been disabled
...
/**
 * Return the next av_frame of a stream.
 ...
 *
 * On success, the returned packet is reference-counted (av_pkt->buf is set) and
 * valid indefinitely. The packet must be freed with av_packet_unref() when
 * it is no longer needed. For video, the packet contains exactly one av_frame.
 * For audio, it contains an integer number of frames if each av_frame has
 * a known fixed size (e.g. PCM or ADPCM data). If the audio frames have
 * a variable size (e.g. MPEG audio), then it contains one av_frame.
 *
 * av_pkt->pts, av_pkt->dts and av_pkt->duration are always set to correct
 * values in AVStream.time_base units (and guessed if the format cannot
 * provide them). av_pkt->pts can be AV_NOPTS_VALUE if the video format
 * has B-frames, so it is better to rely on av_pkt->dts if you do not
 * decompress the payload.
 *
 * @return 0 if OK, < 0 on error or end of file. On error, av_pkt will be blank
 *         (as if it came from av_packet_alloc()).
 *
 * @note av_pkt will be initialized, so it may be uninitialized, but it must not
...
#define AVSEEK_FLAG_ANY      4 ///< seek to any av_frame, even non-keyframes
#define AVSEEK_FLAG_FRAME    8 ///< seeking based on av_frame number
...
/**
 * Write an uncoded av_frame to an output media file.
 *
 * The av_frame must be correctly interleaved according to the container
 ...
 *
 * The caller gives up ownership of the av_frame and must not access it
 * afterwards.
 *
...
/**
 * Test whether a muxer supports uncoded av_frame.
 *
 * @return  >=0 if an uncoded av_frame can be written to that muxer and stream,
 *          <0 if not
 */
 ...
 *              if AVSEEK_FLAG_ANY seek to any av_frame, only keyframes otherwise
...
/**
 * Guess the sample aspect ratio of a av_frame, based on both the stream and the
 * av_frame aspect ratio.
 *
 * Since the av_frame aspect ratio is set by the codec but the stream aspect ratio
 * is set by the demuxer, these two may not be equal. This function tries to
 * return the value that you should use if you would like to display the av_frame.
 *
 * Basic logic is to use the stream aspect ratio if it is set to something sane
 * otherwise use the av_frame aspect ratio. This way a container setting, which is
 * usually easy to modify can override the coded value in the frames.
 *
 * @param format the format context which the stream is part of
 * @param stream the stream which the av_frame is part of
 * @param frame the av_frame with the aspect ratio to be determined
 * @return the guessed (valid) sample_aspect_ratio, 0/1 if no idea
 */
AVRational av_guess_sample_aspect_ratio(AVFormatContext *format, AVStream *stream, AVFrame *frame);

/**
 * Guess the av_frame rate, based on both the container and codec information.
 *
 * @param ctx the format context which the stream is part of
 * @param stream the stream which the av_frame is part of
 * @param frame the av_frame for which the av_frame rate should be determined, may be NULL
 * @return the guessed (valid) av_frame rate, 0/1 if no idea
 */
